...
/**
 * This class represents a routine that can be used by the robot to avoid hitting an obstacle.
 * 
 * @author Team 71
 */
public class ObstacleAvoidance extends Thread {
  ...
  /**
   * This represents the closest an object can be before it is considered to be an emergency.
   */
  ...
  /**
   * Creates an ObstacleAvoidance instance given a Navigation process.
   * 
  ...
  /**
   * When run, the robot will follow the following routine: (1) Turn clockwise until it detects the
   * rightmost edge of the obstacle. (2) Turn past this edge by a small angle calculated to give the
   * robot clearance to get past this edge. (3) Move forward to where the edge was detected, and
   * then continue for a small distance meant to get the body of the robot past the front-most face
   * of the obstacle. (4) Return to the angle it was at when the routine began (5) Move forward
   * slightly, so that the center of rotation of the robot is just past the front-most face (6) Turn
  ...
  /**
   * Allows the robot to find the rightmostface of an obstacle, and move to be alongside it.
   */
  public void movePastEdge() {
    double edgeDist = findEdge();
    double extraT = Math.toDegrees(Math.asin(Lab4.TRACK / 2 / edgeDist));
    nav.turnTo(nav.getOdo().getXYT()[2] + extraT, TURN_SPD);
    moveForward(edgeDist + EXTRA_DIST);
  }

  /**
   * Turns robot clockwise until the edge of the obstacle is detected, then returns the distance of
   * that edge.
   * 
