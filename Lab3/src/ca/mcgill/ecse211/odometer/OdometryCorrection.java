...
/**
 * Represents a thread that uses a light sensor to update
 * the values of a robots odometer to reduce error.
 * @author Group 71
 */
public class OdometryCorrection implements Runnable {

  /**
   * This represents the minimum difference from the mean 
   * for a light sensor reading to be considered significant
   */
  private static final float LIGHT_THRESHOLD = 0.18f;
  /**
   * This represents the distance between lines on
   * the grid, in cm
   */
  private static final float LINE_SPACING = 30.48f;
  /**
   * This represents the minimum distance for the robot
   * to travel before reading another line 
   */
  ...
  /**
   * This is the default class constructor. An existing instance of the odometer is used
   ...
   * This continuously looks for the detection of a line, and 
   * upon seeing one will round either the x or the y position 
   * (whichever is closer) to an integer multiple of the distance
   * between grid lines. 
   * @throws OdometerExceptions
   */
  @Override
  public void run() {
    ...
    while (true) {
      ...
      if (sample[0] < samples.avg - LIGHT_THRESHOLD  && (lastPos == null 
         ...
         * and round that one.
         * We ignore the first line because it is the sensor moving about starting point
         */
        if (lineCount != 1) { 
